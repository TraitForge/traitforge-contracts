...
task('deploy-all', 'Deploy all the contracts').setAction(async (_, hre) => {
  ...
  await hre.run("verify:verify", { address: await token.getAddress() });
  await hre.run("verify:verify", { address: await nft.getAddress() });
  await hre.run("verify:verify", { address: await entropyGenerator.getAddress(), constructorArguments: [await nft.getAddress()] });
  await hre.run("verify:verify", { address: await entityTrading.getAddress(), constructorArguments: [await nft.getAddress()] });
  await hre.run("verify:verify", { address: await entityForging.getAddress(), constructorArguments: [await nft.getAddress()] });
  await hre.run("verify:verify", { address: await devFund.getAddress() });
  await hre.run("verify:verify", { address: await airdrop.getAddress() });
  await hre.run("verify:verify", { address: await daoFund.getAddress(), constructorArguments: [await token.getAddress()] });
  await hre.run("verify:verify", {
    address: await nukeFund.getAddress(),
    constructorArguments: [await nft.getAddress(), await devFund.getAddress(), await airdrop.getAddress(), await daoFund.getAddress()]
  });
...
task('deploy-airdrop', 'Deploy Airdrop')
  .addParam('nft', 'The address of TraitForgeNft')
  .setAction(async (taskArguments, hre) => {
    try {
      ...
      console.log('Contract deployed to:', airdrop.getAddress());
      return airdrop;
    } catch (error) {
      console.error(error);
    }
