...
const entityForgingContract = new ethers.Contract(
  entityForgingAddress,
  entityForgingABI,
  provider
);

const newEntityForgingContract = new ethers.Contract(
...
async function fetchPastEvents() {
  ...
  for (const event of events) {
    ...
    const newTokenid = ethers.BigNumber.from(log.topics[1]).toString();
    const parent1Id = ethers.BigNumber.from(log.topics[2]).toString();
    const parent2Id = ethers.BigNumber.from(log.topics[3]).toString();
    // Récupération du bloc contenant l'événement
    const block = await provider.getBlock(event.blockNumber);
...
async function migrateForgedPairsData(lowerIds: string[], higherIds: string[]) {
    ...
    const contractWithSigner = newEntityForgingContract.connect(signer);
  
    // Appel de la fonction migrateForgedPairsData
    try {
      const tx = await contractWithSigner.migrateForgedPairsData(lowerIds, higherIds);
