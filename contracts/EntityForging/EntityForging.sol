...
contract EntityForging is IEntityForging, ReentrancyGuard, Ownable, Pausable {
  ...
  function forgeWithListed(
    ...
    require(
      forgerTokenId != 0 && mergerTokenId != 0,
      "Invalid token ID: Token ID cannot be 0"
    );
    require(
    ...
    require(
     ...
     uint256 lowerId = forgerTokenId < mergerTokenId ? forgerTokenId : mergerTokenId;
     uint256 higherId = forgerTokenId < mergerTokenId ? mergerTokenId : forgerTokenId; //innefficient
     require(!forgedPairs[lowerId][higherId], "Parents have forged before");


    uint256 forgingFee = _forgerListingInfo.fee;
    require(msg.value == forgingFee, 'Fee cannot be less than or more than msg.value');
